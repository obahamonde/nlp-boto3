...
const fetchResponse = async() => {
    const { data } = await useFetch(`/api/nlp/entities?text=` + model.value.text + '&lang=' + model.value.lang).json()
    response.value = JSON.stringify(unref(data) as  string)
}
...
<div p-32 col center gap-4>
    <h3>Response</h3>
    <textarea no-outline sh p-2 disabled rows="12" cols="48" type="text" v-model="response" />
